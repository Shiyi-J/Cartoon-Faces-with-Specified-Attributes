...
class Discriminator(th.nn.Module):
    ...
    def __init__(self, depth=7, feature_size=512, label_train = False,
        ...
        print('init Dis..., label_train is: ', self.label_train)
        ...
        self.final_block = DisFinalBlock(in_channels=self.feature_size, use_eql=self.use_eql)
        
        
        
        if self.label_train:
            self.face_color = DisFinalBlock(in_channels=self.feature_size, out_channels=11, use_eql=self.use_eql)
            self.glasses = DisFinalBlock(in_channels=self.feature_size, out_channels=2, use_eql=self.use_eql)
    ...
    def forward(self, inputs):
        ...
        if self.label_train:
            return(self.final_block(y),[self.face_color(y), self.glasses(y)])
        # return calculated y
...
class MSG_GAN:
    ...
    def __init__(self, depth=7, latent_size=512,
                 use_eql=True, use_ema=True, ema_decay=0.999,label_train = True,
        ...
        print('init GAN..., label_train is: ', label_train)
        if device == th.device("cuda"):
            self.gen = DataParallel(self.gen)
            self.dis = Discriminator(depth, latent_size, label_train = label_train,
                                     use_eql=use_eql, gpu_parallelize=True).to(device)
        else:
            self.dis = Discriminator(depth, latent_size, label_train = label_train, use_eql=True).to(device)
    ...
    def generate_samples(self, num_samples):
        ...
        noise = self.latent_generator(self, num_samples).to(self.device)
    ...
    def optimize_discriminator(self, dis_optim, noise, real_batch, loss_fn, labels, label_train = False):
        """
        ...
        if label_train:
            ...
            loss_dis, loss_label = loss_fn.dis_loss(real_batch, fake_samples, labels)
            #loss_label = BCEWithLogitsLoss(real_labels, fake_labels)
            
            loss = loss_dis+loss_label
            
            dis_optim.zero_grad()
            loss.backward()
            dis_optim.step()
            
            return loss_dis.item(), loss_label.item()
    ...
    def optimize_generator(self, gen_optim, noise, real_batch, loss_fn, labels, label_train = False):
        """
        ...
        if label_train:
            fake_samples = self.gen(noise)
            loss_dis, loss_label = loss_fn.gen_loss(real_batch, fake_samples, labels)
    ...
    def latent_generator(self, num_samples, normalize_latents = True):
        latent = th.randn(num_samples, self.latent_size)
        if normalize_latents:
            latent = (latent
                           / latent.norm(dim=-1, keepdim=True)
                           * (self.latent_size ** 0.5))
        #if self.label_train:
        latent[:, self.latent_size-2] = th.randint(size=(1,num_samples), low=0, high=11) # face color 
        latent[:, self.latent_size-1] = th.randint(size=(1,num_samples), low=0, high=2) # glasses
    ...
    def train(self, data, gen_optim, dis_optim, loss_fn, normalize_latents=True,
        ...
        fixed_input = self.latent_generator(num_samples, normalize_latents)
        #if normalize_latents:
        ...
        for epoch in range(start, num_epochs + 1):
            ...
            for (i, batch) in enumerate(data, 1):
                ...
                if self.label_train:
                    images = batch[0].to(self.device) # size B x C x H x W
                    #real_labels = batch[1].to(self.device) # size B
                    real_labels = self.labels_to_device(batch[1])
                ...
                #                 * (self.latent_size ** 0.5))
                gan_input = self.latent_generator(extracted_batch_size, normalize_latents)
                f_labels = gan_input[:,self.latent_size-2:self.latent_size].to(self.device)
                fake_labels = []
                fake_labels.append(f_labels[:, 0])
                fake_labels.append(f_labels[:, 1])
                
                # optimize the discriminator:
                if self.label_train:
                    dis_loss, dis_label_loss = self.optimize_discriminator(dis_optim, gan_input,
                                                           images, loss_fn, real_labels, self.label_train)

                    # optimize the generator:
                    gen_loss, gen_label_loss = self.optimize_generator(gen_optim, gan_input,
                                                           images, loss_fn, fake_labels, self.label_train)
                    if dis_label_loss is None:
                        dis_label_loss = 123
                        gen_label_loss = 123
                    ...
                    if i % (int(limit / feedback_factor) + 1) == 0 or i == 1:     # Avoid div by 0 error on small training sets
                        ...
                        print("Elapsed [%s] batch: %d  d_loss: %f dis_label_loss: %f g_loss: %f gen_label_loss: %f"
                              % (elapsed, i, dis_loss, dis_label_loss, gen_loss, gen_label_loss))
                        ...
                        if log_dir is not None:
                            ...
                            with open(log_file, "a") as log:
                                log.write(str(global_step) + "\t" + str(dis_loss) + "\t" + str(dis_label_loss) +
                                          "\t" + str(gen_loss) +"\t" + str(gen_label_loss) + "\n")
                    ...
                    dis_loss = self.optimize_discriminator(dis_optim, gan_input,
                                                           images, loss_fn, None, self.label_train)
