...
class Label_RelativisticAverageHingeGAN(GANLoss):

    def __init__(self, dis):
        super().__init__(dis)
        self.label_loss = th.nn.BCEWithLogitsLoss()
        self.label_loss_1 = th.nn.CrossEntropyLoss()
    def dis_loss(self, real_samps, fake_samps, labels):
        # Obtain predictions
        r_preds, r_labels = self.dis(real_samps)
        f_preds, f_labels = self.dis(fake_samps)
        #print('r_labels type: ', type(r_labels))
        #labels = labels.float()
        # difference between real and fake:
        r_f_diff = r_preds - th.mean(f_preds)
        ...
        ###label_loss = self.label_loss(r_labels, labels)
        #print(r_labels.shape)
        #print(labels.shape)
        label_loss = self.label_loss_1(r_labels[0], labels[0].float().type(th.long))
        label_loss += self.label_loss_1(r_labels[1], labels[1].float().type(th.long))
        #print(label_loss)
    ...
    def gen_loss(self, real_samps, fake_samps, labels):
        ...
        ##label_loss = self.label_loss(f_labels, labels)
        label_loss = self.label_loss_1(f_labels[0], labels[0].float().type(th.long))
        label_loss += self.label_loss_1(f_labels[1], labels[1].float().type(th.long))
        return loss, label_loss